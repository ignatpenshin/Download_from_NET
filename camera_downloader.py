...
def insta_cleaner(sftp, pairs, prod = False): 
    print("----- INSTA_CLEANER ======")
    for cam_photo in pairs:
        if os.path.exists(pairs[cam_photo]):
            if sftp.exists(cam_photo):
                sftp.remove(cam_photo)
            else: 
                print("lol, file not found on INSTA")
        else: 
            if sftp.exists(cam_photo):
                print('loading lost file')
                sftp.get(cam_photo, pairs[cam_photo], callback = lambda x,y: progressbar(x,y))
            else:
                print('wtf')
    ...
    if prod != True:
        print("-->> UPLOAD AGAIN <<--")
        for cam_photo in pairs:
            sftp.put(pairs[cam_photo], cam_photo, callback = lambda x,y: progressbar(x,y))
...
def camera_process(exiftool_path, exiftool_runner, stitch_path, stitch_runner, basic_dir, Hostname, Username, Password, cnopts, \
    ...
    with pysftp.Connection(host=Hostname, username=Username, password=Password, cnopts=cnopts) as sftp:
        ...
        for Camera_n in directory_structure:
            ...
            for file in [x for x in sorted(sftp.listdir_attr(),  
                ...
                if str(cameras[cam_path][0] + file_date[:-7]) != dir_to_create: 
                    ...
                    sftp.cwd(cam_path_way + "/" + Camera_n.filename)
                pairs[cam_path_way + "/" + Camera_n.filename + "/" + file.filename] = path_log+"\\original\\"+file.filename
        ...
        #Control with remove
        if len(pairs) == image_counter:
            insta_cleaner(sftp, pairs)
    ...
    with pysftp.Connection(host=Hostname, username=Username, password=Password, cnopts=cnopts) as sftp:
        ...
        if argv == None:
            for cam_path in cam_list:
                os.system("start cmd /c " + os.path.basename(sys.argv[0]) + " " + cam_path.filename) 
